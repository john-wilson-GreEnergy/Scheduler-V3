...
export default function NotificationPanel({ employeeId, onClose }: NotificationPanelProps) {
  ...
  return (
    ...
    >
      ...
      <div className="px-6 py-3 border-b border-emerald-900/10 flex gap-2 bg-[#0A120F]/50">
        <button 
          onClick={() => setFilter('all')}
          className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider transition-all ${
            filter === 'all' ? 'bg-emerald-500 text-black' : 'bg-white/5 text-gray-400 hover:bg-white/10'
          }`}
        >
          All
        </button>
        <button 
          ...
          className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider transition-all ${
            filter === 'unread' ? 'bg-emerald-500 text-black' : 'bg-white/5 text-gray-400 hover:bg-white/10'
          }`}
        >
