...
export default function BulkAssignments({ employees, jobsites }: BulkAssignmentsProps) {
  ...
  const plannedChanges = useMemo(() => {
    ...
    return changes;
  }, [selectedEmployees, selectedJobsite, selectedWeeks, employees, jobsites]);
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      // 2. Get the IDs of the created/updated weeks to link items
      ...
      if (weeks) {
        const items = plannedChanges.map(change => {
          const emp = employees.find(e => e.id === change.employeeId)!;
          ...
          return {
            ...
            jobsite_fk: change.jobsiteId,
            customer: jobsite.customer,
            normalized_value: change.jobsiteName,
            item_type: 'jobsite',
            item_order: 0
          };
        }).filter(item => item.assignment_week_fk);

        const { error: itemError } = await supabase
          .from('assignment_items')
          .upsert(items, { onConflict: 'assignment_week_fk' });
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="bg-[#0A120F] border border-emerald-900/30 rounded-3xl p-6">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {jobsites.filter(j => j.is_active).map(site => (
                  <button
                    key={site.id}
            ...
            <div className="bg-[#0A120F] border border-emerald-900/30 rounded-3xl p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {nextFourWeeks.map(week => (
                  ...
                  >
                    <p className="text-[10px] font-bold text-white">{format(new Date(week), 'MMM dd')}</p>
