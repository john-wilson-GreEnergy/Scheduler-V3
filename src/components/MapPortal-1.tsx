...
import { MapPin, Navigation, Phone, User, ChevronRight, ChevronDown, Filter, ArrowUpDown, Hexagon, Construction, Users, Maximize2, RefreshCw, Calendar } from 'lucide-react';
...
export default function MapPortal({ jobsites, employees: providedEmployees }: MapPortalProps) {
      ...
      const processWeek = (week: any) => {
        ...
        // 2. Fallback to assignment_name matching (legacy/fallback system)
          ...
          if (OFF_SITE_STATUSES.includes(trimmedName)) {
            addToOffSite(week.assignment_name.trim(), employee);
            return;
          }
  ...
  return (
    <div className="h-[calc(100vh-200px)] rounded-3xl overflow-hidden border border-emerald-900/30 relative map-portal-container">
      <style>{`
      ...
      <div className="absolute top-6 left-6 z-[1000]">
        <div className="bg-[#050A08]/90 backdrop-blur-xl border border-emerald-500/20 rounded-2xl shadow-2xl p-2 flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-emerald-500/10 flex items-center justify-center">
            <Calendar className="text-emerald-500" size={20} />
          </div>
          <div className="pr-4">
            <label className="block text-[8px] font-black text-emerald-500/50 uppercase tracking-tighter mb-0.5">Viewing Logistics For</label>
            <select
              value={selectedWeek}
              onChange={(e) => setSelectedWeek(e.target.value)}
              className="bg-transparent text-white text-xs font-bold outline-none cursor-pointer appearance-none hover:text-emerald-400 transition-colors"
            >
              {mondays.map(monday => (
                <option key={monday.toISOString()} value={format(monday, 'yyyy-MM-dd')} className="bg-[#0A120F] text-white">
                  Week of {format(monday, 'MMM dd, yyyy')}
                </option>
              ))}
            </select>
          </div>
          ...
          <button 
            ...
            className="px-3 py-2 text-[9px] font-black text-gray-500 hover:text-emerald-500 uppercase tracking-widest transition-colors"
          >
            Current
          </button>
        </div>
      ...
      <div className={`absolute top-6 right-6 z-[1000] transition-all duration-500 ease-in-out ${legendOpen ? 'w-80' : 'w-14'}`}>
        <div className="bg-[#050A08]/90 backdrop-blur-xl border border-emerald-500/20 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden">
          <button 
            onClick={() => setLegendOpen(!legendOpen)}
            className="w-full p-5 flex items-center justify-between hover:bg-emerald-500/5 transition-colors group"
          >
            <div className="flex items-center gap-4">
              <div className="relative">
                <div className="w-10 h-10 rounded-2xl bg-emerald-500/10 flex items-center justify-center group-hover:scale-110 transition-transform">
                  <Hexagon size={20} className="text-emerald-500 fill-emerald-500/10" />
                  <Construction size={10} className="text-emerald-500 absolute" />
                </div>
              </div>
              {legendOpen && (
                <div className="text-left">
                  <span className="block font-black text-white text-sm uppercase tracking-widest">GreEnergy</span>
                  <span className="block text-[10px] text-emerald-500/50 font-bold uppercase tracking-tighter">Logistics Fleet</span>
                </div>
              )}
            </div>
            {legendOpen ? <ChevronRight size={18} className="text-gray-500" /> : null}
          </button>
      ...
      {/* Staffing Legend */}
      <div className="absolute bottom-6 left-6 z-[1000] bg-[#0A120F]/90 backdrop-blur-md border border-white/10 p-4 rounded-2xl shadow-2xl">
